...
### Kubernetes
Kubernetes is known as the only reasonable way to orchestrate containerized applications in a large scale.
...
Now let's try to answer the question "Why we use it in scope of our experiments"?
...
Thankfully, there is a tool that can makes your life much easier - `Ansible`.
Ansible is a declarative way how to write automation scrips in yaml format, which also allows you to use libraries with already implemented steps and tasks that interact with many components, so you do not need re-invent the wheel.
Because the initial installation path of our environment requires a lot of steps, Ansible it the correct way to deduplicate steps, create a templates in `jinja` and drive execution based on tags.
...
Let me explain it on simple example.
Imagine pipeline, which does an API request using `curl` and then execute `maven` command.
...
Basically if someone put changes of application configuration into git repository, ArgoCD gets this changes and apply them to target kubernetes cluster.
